...
namespace Erikwang2013\IndustrialProtocols\CanOpen;
...
class CanOpenProtocol implements ProtocolInterface
{
    ...
    public function createConnector(array $config): ConnectorInterface
    {
        if (!isset($config['bridge'])) {
            throw new \RuntimeException("CANopen requires a BridgeInterface in config['bridge']");
        }
        return new BridgeConnector($config['bridge'], 'canopen');
    }
}
