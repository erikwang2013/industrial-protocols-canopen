...
CANopen 协议包 — 需 PCAN-USB/SocketCAN 接口桥接。Requires dedicated hardware, bridged to vendor SDK or gateway via kernel Bridge layer.

## Installation

```bash
composer require erikwang2013/industrial-protocols-kernel erikwang2013/industrial-protocols-canopen
```
...
$conn = $kernel->getConnectionManager()->connect('device-id', [
    'protocol' => 'canopen', 'bridge' => $bridge,
]);
$result = $conn->read('address');

// Via vendor factory
$bridge = $kernel->getVendorBridgeFactory()->create('hilscher', 'cifX CAN', '');
```

## Adapter Vendors

Hilscher (cifX CAN), HMS/Anybus (CANopen Slave), Peak System (PCAN-USB), IXXAT
...
- CAN interface (PCAN-USB, IXXAT USB-to-CAN, SocketCAN)
